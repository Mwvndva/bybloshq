...
export default function EventsListPage() {
  ...
  const calculateRevenueBreakdown = (event: Event) => {
    const totalRevenue = (event.tickets_sold || 0) * (event.ticket_price || 0);
  ...
  const processWithdrawal = async () => {
    ...
    if (withdrawMethod === 'mpesa') {
      ...
      if (!withdrawDetails.bankName || !withdrawDetails.accountNumber) {
        toast({
          title: "Validation Error",
          description: "Please fill in bank name and account number",
          variant: "destructive"
        });
        return;
      }
    ...
    try {
      ...
      const emailData = {
        ...
        html: `
          ...
          <h3>Event Details:</h3>
          <ul>
            <li><strong>Event Name:</strong> ${selectedEventForWithdraw.name}</li>
            <li><strong>Event ID:</strong> ${selectedEventForWithdraw.id}</li>
            <li><strong>Location:</strong> ${selectedEventForWithdraw.location}</li>
            <li><strong>Date:</strong> ${formatEventDate(selectedEventForWithdraw.start_date)}</li>
          </ul>
          
          <h3>Financial Breakdown:</h3>
          <ul>
            <li><strong>Total Revenue:</strong> KSh ${breakdown.totalRevenue.toLocaleString('en-KE')}</li>
            <li><strong>Platform Fee (6%):</strong> KSh ${breakdown.platformFee.toLocaleString('en-KE')}</li>
            <li><strong>Net Payout (94%):</strong> KSh ${breakdown.netPayout.toLocaleString('en-KE')}</li>
          </ul>
          
          <h3>Withdrawal Details:</h3>
          <ul>
            <li><strong>Method:</strong> ${withdrawMethod === 'mpesa' ? 'M-Pesa' : 'Bank Transfer'}</li>
            ${withdrawMethod === 'mpesa' ? `
              <li><strong>M-Pesa Number:</strong> ${withdrawDetails.mpesaNumber}</li>
              <li><strong>Registered Name:</strong> ${withdrawDetails.registeredName}</li>
            ` : `
              <li><strong>Bank Name:</strong> ${withdrawDetails.bankName}</li>
              <li><strong>Account Number:</strong> ${withdrawDetails.accountNumber}</li>
            `}
          </ul>
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to process withdrawal request';
      toast({
        title: "Error",
                      ...
                      <div className="mt-4 space-y-2">
                        ...
                        {event.withdrawal_status === 'paid' ? (
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            className="w-full border-yellow-200 hover:bg-yellow-50 hover:border-yellow-300 rounded-xl text-yellow-700 hover:text-yellow-800"
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setWithdrawDetails(prev => ({ ...prev, bankName: e.target.value }))}
                        className="border-gray-200 rounded-xl"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setWithdrawDetails(prev => ({ ...prev, accountNumber: e.target.value }))}
                        className="border-gray-200 rounded-xl"
                      />
                    </div>
                  </div>
