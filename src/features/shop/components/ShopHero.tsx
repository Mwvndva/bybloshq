    ...
    <header className="relative w-full max-w-4xl mx-auto px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 sm:pt-6 sm:pb-4">
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <Button
          ...
          variant="ghost"
          size="sm"
          className="rounded-full px-3 py-1.5 text-xs font-bold text-slate-700 hover:text-slate-950 hover:bg-slate-200/60 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/10 transition-colors flex items-center gap-1.5"
        >
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <div className="relative mb-3">
          <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full overflow-hidden border-4 border-yellow-400 bg-slate-100 dark:bg-zinc-900 shadow-xl flex items-center justify-center text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">
            {showSellerAvatar ? (
              <img
        ...
        {/* Shop Name */}
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-black tracking-tight text-slate-950 dark:text-white max-w-xl px-2">
          {sellerInfo?.shopName || 'Shop'}
        </h1>

        {/* Shop Bio */}
        {sellerInfo?.bio && (
          <p className="mt-1 max-w-lg text-xs sm:text-sm font-medium text-slate-600 dark:text-white/70 leading-relaxed px-4">
            {sellerInfo.bio}
          </p>
        )}
        ...
        <div className="mt-3 flex flex-wrap items-center justify-center gap-2 text-xs font-semibold">
          {/* Followers */}
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-800 border border-slate-200 dark:bg-white/5 dark:text-white/90 dark:border-white/10 shadow-sm" title="Followers">
            <Users className="h-3.5 w-3.5 text-yellow-500" />
            <span>{sellerInfo?.clientCount || 0} followers</span>
          </span>

          {/* Shop Type */}
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-800 border border-slate-200 dark:bg-white/5 dark:text-white/90 dark:border-white/10 shadow-sm" title="Shop Type">
          ...
          {sellerInfo?.instagramLink && (
            <a
              ...
              title="Instagram"
              className="p-1.5 rounded-full bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 hover:text-slate-950 dark:bg-white/5 dark:hover:bg-white/15 dark:border-white/10 dark:text-white/80 dark:hover:text-white transition-all shadow-sm"
            >
