...
import { ProductImageViewer } from './product-card/ProductImageViewer';
import { ProductCardModals } from './product-card/ProductCardModals';
import { createCheckoutAttemptToken, getProductCardThemeVars, getProductFlags, getThemeClasses, normalizePhone, type Theme } from './product-card/productCardUtils';
...
export function ProductCard({ product, seller, hideWishlist = false, theme, forceWhiteText = false }: ProductCardProps) {
  ...
  return (
    <Card
      className={cn(
        'group relative flex h-full min-h-[320px] flex-col overflow-hidden transition-all duration-300 rounded-xl sm:rounded-2xl sm:min-h-[420px]',
