...
export function EventForm({ defaultValues, onSubmit, isSubmitting }: EventFormProps) {
  // Check if we're in edit mode (if defaultValues has an id, it's an existing event)
  const isEditMode = !!(defaultValues as any)?.id;
  
  // Initialize image preview from either a File object or an image URL
  const [imagePreview, setImagePreview] = useState<string | null>(
    defaultValues?.image 
      ? URL.createObjectURL(defaultValues.image as unknown as Blob) 
      : (defaultValues as any)?.image_url || null
  );
  ...
  const removeImage = () => {
    ...
    // Also clear image_url if it exists in defaultValues
    if ((defaultValues as any)?.image_url) {
      setValue('image', undefined);
    }
  };
