...
const transformProduct = (product: unknown): Product | null => {
  ...
  const transformedProduct: Product = {
    ...
    image_url: String(pObj.image_url || pObj.imageUrl || 'https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&w=800&q=80'),
    sellerId: String(pObj.sellerId || pObj.seller_id || ''),
    isSold: Boolean(pObj.isSold || pObj.status === 'sold'),
    status: (pObj.status || (pObj.isSold ? 'sold' : 'available')) as Product['status'],
    soldAt: (pObj.soldAt || pObj.sold_at || null) as Product['soldAt'],
    createdAt: String(pObj.createdAt || pObj.created_at || new Date().toISOString()),
    updatedAt: String(pObj.updatedAt || pObj.updated_at || new Date().toISOString()),
    aesthetic: (pObj.aesthetic || 'noir') as Aesthetic,
    is_digital: (pObj.is_digital || pObj.isDigital) as Product['is_digital'],
    product_type: (pObj.product_type || pObj.productType || 'physical') as Product['product_type'],
    service_options: (pObj.service_options || pObj.serviceOptions) as Product['service_options'],
    service_locations: (pObj.service_locations || pObj.serviceLocations) as Product['service_locations'],
  };
