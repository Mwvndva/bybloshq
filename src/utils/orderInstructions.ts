...
export const getOrderInstruction = ({
    ...
    const instructions: Record<string, Record<string, OrderInstructionResult>> = {
        buyer: {
            ...
            FULFILLING: { text: isService ? 'Booking confirmed. Service is in progress.' : isCourier ? 'Package movement is in progress.' : 'Seller is preparing your pickup.', color: 'amber' },
            READY_FOR_BUYER: { text: isShopPickup ? 'Ready! Visit the shop to collect your order.' : `Ready at hub. Collect at ${COURIER_LOCATION}.`, color: 'amber' },
        ...
        seller: {
            ...
            FULFILLING: { text: isService ? 'Complete the service, then mark it completed.' : isCourier ? 'Complete your selected handoff.' : 'Prepare item for buyer collection.', color: 'amber' },
            READY_FOR_BUYER: { text: 'Buyer has been notified. Complete handover when they arrive.', color: 'amber' },
