...
export const getOrderInstruction = ({
    ...
    const instructions: Record<string, Record<string, OrderInstructionResult>> = {
        buyer: {
            ...
            AWAITING_SELLER_ACTION: { text: isService ? 'Waiting for seller to confirm your booking.' : isCourier ? 'Seller will choose hub drop-off or Mzigo pickup.' : 'Seller is preparing your order.', color: 'amber' },
            FULFILLING: { text: isService ? 'Booking confirmed. After the service is complete, mark it completed to release funds.' : isCourier ? 'Package movement is in progress.' : 'Seller is preparing your pickup.', color: 'amber' },
            READY_FOR_BUYER: { text: isService ? 'Service delivered. Mark it completed to release funds.' : isShopPickup ? 'Ready! Visit the shop to collect your order, then confirm receipt.' : `Ready at hub. Collect at ${COURIER_LOCATION}, then confirm receipt.`, color: 'amber' },
        ...
        seller: {
            PENDING: { text: isCourier ? `Drop off item at ${COURIER_LOCATION} within 24 hours.` : isShopPickup ? 'Prepare item for buyer collection.' : isService ? 'New booking! Confirm the appointment.' : 'New order received.', color: 'blue' },
            ...
            AWAITING_SELLER_ACTION: { text: isService ? 'Confirm this booking to proceed.' : isCourier ? 'Choose hub drop-off or request Mzigo pickup.' : 'Prepare item for buyer collection.', color: 'amber' },
            FULFILLING: { text: isService ? 'Deliver the service as scheduled. The buyer will mark it completed to release funds.' : isCourier ? 'Complete your selected handoff.' : 'Prepare item for buyer collection.', color: 'amber' },
