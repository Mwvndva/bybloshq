...
function walk(dir, depth = 0) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) files.push(...walk(full, depth + 1));
...
const graph = {};
const allRels = new Set(files.map(f => path.relative(root, f).replace(/\\/g, '/')));
...
const hubs = [...nodes]
  ...
  .slice(0, 20)
  .map(n => ({ id: n.id, importedBy: n.importedBy.length, imports: n.imports.length }));

// Build API surface (server routes/controllers)
const serverFiles = nodes.filter(n => n.role === 'server').map(n => n.id);
...
console.log('Top hubs:', hubs.slice(0, 5).map(h => h.id + ' (' + h.importedBy + ' importers)').join(', '));
