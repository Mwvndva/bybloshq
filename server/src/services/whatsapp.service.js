...
class WhatsAppService {
    constructor() {
        ...
        this.isReady = false;
        this.qrCode = null;
        this.isInitializing = false; // Guard against concurrent initializations
    ...
    async initialize() {
        try {
            ...
            if (this.isInitializing) {
                console.log('⚠️ WhatsApp initialization already in progress, skipping...');
                return;
            }

            if (this.isReady) {
                console.log('ℹ️ WhatsApp client is already ready');
                return;
            }
            ...
            if (this.client) {
                try {
                    ...
                    console.warn('⚠️ Non-critical error during client cleanup:', cleanupError.message);
                }
                this.client = null;
            }
    ...
    async logout() {
        try {
            if (this.client) {
                ...
                // Use a timeout for logout as it can hang or throw EBUSY
                try {
                    const timeoutPromise = new Promise((_, reject) =>
                        setTimeout(() => reject(new Error('Logout timed out')), 10000)
                    );
                    await Promise.race([this.client.logout(), timeoutPromise]);
                    console.log('✅ WhatsApp logout call completed');
                } catch (logoutError) {
                    console.error('⚠️ WhatsApp logout error (possibly EBUSY):', logoutError.message);
                    // We continue to destroy anyway
                }
                ...
                this.isReady = false;
                this.qrCode = null;
                console.log('✅ WhatsApp service state reset successfully');
            } else {
                console.log('ℹ️ WhatsApp logout called but no client exists');
            }
        } catch (error) {
            console.error('❌ Fatal error in WhatsApp logout:', error);
            // Ensure state is at least partially reset
            this.isReady = false;
            this.client = null;
        }
    }
