...
export const updateEvent = async (req, res) => {
  ...
  try {
    ...
    // If ticket_types is a string (from FormData), parse it
    ...
    // Handle image - use image_data_url if provided
    let finalImageUrl = image_data_url;
    ...
    if (finalImageUrl) {
      ...
      // Validate image size (max 2MB)
      const imageSize = (finalImageUrl.length * 0.75); // Convert base64 to bytes
      if (imageSize > 2 * 1024 * 1024) { // 2MB
        await client.query('ROLLBACK');
        return res.status(400).json({
          status: 'error',
    ...
    const startDate = start_date ? new Date(start_date) : new Date(event.start_date);
    const endDate = end_date ? new Date(end_date) : new Date(event.end_date);
    ...
    if (venue !== undefined) {
      // Note: venue might not be a column, check your schema
      // For now, we'll skip it if venue column doesn't exist
    }
    ...
    if (is_online !== undefined) {
      ...
      queryParams.push(is_online === 'true' || is_online === true);
    }
    if (online_url !== undefined) {
      updateFields.push(`online_url = $${paramIndex++}`);
      queryParams.push(online_url);
    }

    // Update ticket_quantity and ticket_price only if not using ticket types
    if (ticket_types && Array.isArray(ticket_types) && ticket_types.length > 0) {
      // If ticket types are provided, set ticket_quantity to 0 (indicates ticket types are used)
      updateFields.push(`ticket_quantity = $${paramIndex++}`);
      queryParams.push(0);
      
      // Calculate min price from ticket types
      const minPrice = Math.min(...ticket_types.map(t => parseFloat(t.price || 0)));
    ...
    if (ticket_types && Array.isArray(ticket_types) && ticket_types.length > 0) {
      // Delete existing ticket types that are not in the update
      const existingTicketTypes = await client.query(
        'SELECT id FROM ticket_types WHERE event_id = $1',
        [id]
      );
      const existingIds = existingTicketTypes.rows.map(r => r.id.toString());
      const updateIds = ticket_types
        .filter(t => t.id)
        .map(t => t.id.toString());
      ...
      if (idsToDelete.length > 0) {
        await client.query(
          'DELETE FROM ticket_types WHERE id = ANY($1::int[])',
          [idsToDelete]
        );
      }
      ...
      for (const ticketType of ticket_types) {
        ...
        if (ticketId && existingIds.includes(ticketId.toString())) {
          // Update existing ticket type
          await client.query(
            `UPDATE ticket_types 
             SET name = $1, price = $2, quantity = $3, description = $4,
                 sales_start_date = $5, sales_end_date = $6, updated_at = NOW()
             WHERE id = $7`,
            [
              ticketName,
              ...
              sales_start_date ? new Date(sales_start_date) : null,
              sales_end_date ? new Date(sales_end_date) : null,
              ticketId
            ]
          );
        } else {
          ...
          await client.query(
            `INSERT INTO ticket_types (
              event_id, name, price, quantity, description,
    ...
    const updatedEventWithTypes = await Event.findById(id);
    
    res.status(200).json({
      status: 'success',
      data: {
        event: updatedEventWithTypes
      }
    });
  } catch (error) {
    await client.query('ROLLBACK');
