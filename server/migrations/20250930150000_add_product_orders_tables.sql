...
-- Fix case sensitivity issues with status values (moved from 20250924000000_fix_payment_status_case.sql)
-- Ensure all status values are UPPERCASE for consistency
UPDATE product_orders 
SET status = UPPER(status::text)::order_status
WHERE status::text != UPPER(status::text);

-- Add comments to document the status column conventions
COMMENT ON COLUMN product_orders.status IS 'Order status (PENDING, COMPLETED, FAILED, etc.) - UPPERCASE';
COMMENT ON COLUMN product_orders.payment_status IS 'Payment status (pending, paid, failed, etc.) - lowercase';
