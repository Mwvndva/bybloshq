-- Migration: Add manual_review status, order_id, and notes to refund_requests table
-- Reason: Surface compensation_required and manual_review orders in Admin Dashboard

-- 1. Add order_id and notes columns if they do not exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
INSERT INTO refund_requests (buyer_id, order_id, amount, status, notes, payment_method, payment_details, requested_at)
SELECT
    po.buyer_id,
    po.id AS order_id,
